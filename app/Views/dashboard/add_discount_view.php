    ...
    <div class="editFields fs-add-discount-box">
        <form name='addDiscountForm' method='post' action='' id='addDiscountForm'>
           <input type="hidden" class="textfield" id="hiddenEventEndDate" name="hiddenEventEndDate" value="<?php if (isset($discountDetails['response']['discountList'][0]['startdatetime'])) {
            ...
            <?php if($masterEvent == TRUE): ?>
            <label>Level<span class="mandatory">*</span></label>
            <select name="discountLevel" name="discountLevel">
                <?php if((isset($discountDetails) && $discountDetails['response']['discountList'][0]['type'] == 'masterDiscount' && $discountDetails['response']['discountList'][0]['eventid'] == $eventId) || isset($discountDetails) == FALSE){?>
                    <option value="<?php echo $eventId; ?>">All Events</option>
                <?php } ?>
                <?php foreach($childEvents as $event): ?>
                <?php if(isset($discountDetails) == TRUE && $discountDetails['response']['discountList'][0]['type'] == 'normal' && $discountDetails['response']['discountList'][0]['eventid'] == $event['id']){ ?>
                    <option value="<?php echo $event['id']; ?>"><?php echo allTimeFormats($event['startDateTime'], 3); ?></option>
                <?php } elseif(isset($discountDetails) == FALSE){?>
                    <option value="<?php echo $event['id']; ?>"><?php echo allTimeFormats($event['startDateTime'], 3); ?></option>
                <?php } ?>
                <?php endforeach; ?>
            </select>
            ...
            <input type="text" class="textfield" id="discountCode" name="discountCode" <?php if (isset($discountDetails['response']['discountList'][0]['code'])) { ?>readonly value="<?php echo $discountDetails['response']['discountList'][0]['code'] ?>"<?php } ?> >
