...
<?php }else{  ?>
                ...
                foreach ($transactionList as $value) {
                    ...
                    foreach ($value['ticketDetails'] as $key => $ticket) {
                        ...
                        <tr>
                            ...
                            <?php } ?>
                            <td><?php
                                echo $ticket['Name'] . '<br>' . $ticket['Email'] . '<br>' . $ticket['Mobile'];
                            ...
                            <?php } elseif ($transactionType == 'incomplete') {
                                ...
                                <!-- <td><?php //echo ($bookedTicketsCount == $loop) ? $value['failedcount'] : '';
                                    ...
                                    <?php 
                                        if(($eventId == 234986 || $eventId == 240049 || $eventId == 245850) && ($bookedTicketsCount == $loop))
                                        {
                                            echo "<td>".$value['failedcount']."</td>";
                                        }
                                    ?>

                                <td><?php if($bookedTicketsCount == $loop){
                                        if(isset($value['comment']) && !empty($value['comment'])){
                                            foreach ($value['comment']  as $vcmtkey => $vcmtvalue) {
                                                echo ($vcmtkey+1).'. '.$vcmtvalue."<br>";
                                            }
                            ...
                            <td>
                                <?php 
                                if($bookedTicketsCount == $loop){?><a href='<?php echo commonHelperGetPageUrl('dashboard-ticketTransfer',$eventId.'&'.$value['id']);?>' >
                                <?php if(!empty($GPTW_EVENTS_ARRAY[$eventId])){ echo "Edit Attendee Details"; } else { ?> <span class="icon-edit"></span> <?php } ?> </a><br /><?php }?>
                                     <?php    if($reportType == 'summary'){ 
                                         
                                                if(!empty($GPTW_EVENTS_ARRAY[$eventId]))
                                                {                                                    
                                         ?>
                                        <a href='<?php echo commonHelperGetPageUrl('dashboard-edit-offline-purchaser', $value['id']);?>' title="Edit Details" >Edit invoice</a><br />
                                        <?php
                                                }
                                        if($GPTW_transactions[$value['id']]['raise_tax_invoice'] != 1 && !empty($GPTW_EVENTS_ARRAY[$eventId])) {
                                            ?>
                                        <a href="#" onclick="return func_confirm_raise_tax_invoice('<?php echo commonHelperGetPageUrl('raise-tax-invoice', $value['id']); ?>')" > Raise Tax Invoice</a> <br />
                                        <?php
                                        }
                                        else
                                        {
                                            ?>
                                            <a title = "Resend Email" class="resendEmail" value="<?php echo $value['id']; ?>" href="javascript:void()"><span id="resendEmailSuccessMessage<?php echo $value['id']; ?>" ></span>
                                            <?php if(!empty($GPTW_EVENTS_ARRAY[$eventId])){ echo "Email Tax Invoice"; } else { ?> <span style="float:inline" class ="icon-mail"></span><?php } ?>
                                            </a><br />
                                            <?php
                                        }
                                        ?>
                                    ...
                                    <?php
                                      if(!empty($GPTW_EVENTS_ARRAY[$eventId]))
                                        {
                                            ?>
                                            <a href='<?php echo commonHelperGetPageUrl('dashboard-download-tax-invoice', $value['id']);?>' >View Tax Invoice </a><br />
                                            <a href='<?php echo commonHelperGetPageUrl('dashboard-download-attendee-info', $value['id']);?>' > Download Details</a><br />
                                        <?php
                                        }
                                        ?>
                    ...
                    <?php } ?>
                                <?php if ($transactionType == 'incomplete') { ?>
                                <a href='<?php echo commonHelperGetPageUrl('dashboard-incompletetransaction-comment',$eventId.'&'.$value['id']);?>' >Add Comment</a>
                                <?php } ?>
                            </td>
    ...
    <?php }?>
     <?php }else{?>
                <?php
                $i = 1;
         foreach($grandTotal['response']['transactionList'] as $GPTW_transactions[$value['id']]){
             ?>
        <tr>
                        <td><?php echo $i++; ?></td>
                        <td><?php echo date($GPTW_transactions[$value['id']]['eventsignupid']); ?></td>
                        <td><?php echo $GPTW_transactions[$value['id']]['signupdate']; ?></td>
                        <td><?php echo $GPTW_transactions[$value['id']]['ticketname']; ?></td>
                        <!-- <td><?php //echo $GPTW_transactions[$value['id']]['transactiontickettype']; ?></td> -->
                        <td><?php echo $GPTW_transactions[$value['id']]['name'] . '<br>' . $GPTW_transactions[$value['id']]['email'] . '<br>' . $GPTW_transactions[$value['id']]['mobile']; ?></td>
                        <td><?php echo $GPTW_transactions[$value['id']]['quantity']; ?></td>
                        <td><?php echo $GPTW_transactions[$value['id']]['pg_status']; ?></td>                       
                        <td><?php echo $GPTW_transactions[$value['id']]['pg_amount']; ?></td>
                        <td><?php echo $GPTW_transactions[$value['id']]['pg_message']; ?></td>
