...
<div class="rightArea">
    ...
    <?php if($this->customsession->getData('planFlashMessage')){ ?>
    <div class="success db-alert db-alert-success flashHide"> 
        <?php echo $this->customsession->getData('planFlashMessage'); $this->customsession->unSetData('planFlashMessage');  ?>
    </div>
    <?php } ?>
    <!--Data Section Start-->
    <!--
                           ...
                           <?php $SNo = 1;
                            ...
                            foreach ($searchData as $key => $value) {
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $value['currencyid']=='10' ? "GBP" : ""; ?></td>
                                    <td>
                                         <a title="Edit Plan" class="tooltip-left hoeverclass" data-tooltip="Edit Plan" 
                                        href="<?php echo base_url()."dashboard/recurringpayment/plan/edit/".$value['id']?>"><span class="fa fa-edit gridview-edit"></span></a>
                                        <a title="Delete Plan" class="tooltip-left hoeverclass" data-tooltip="Delete Plan" onclick="deletePlan(this)" data-id="<?php echo $value['id']?>" href="javascript:void(0);"><span class="fa fa-trash gridview-delete"></span></a>
                                    </td>
                                </tr>
                            <?php  $loop++; } ?>
                    <?php } ?>
...
<script type="text/javascript">
    var delUrl = '<?php echo base_url()."dashboard/recurringpayment/plan/delete/"?>';
</script>
    
  <script type="text/javascript">
    function deletePlan(comp) {
        ...
        {   
            ...
            $.ajax({
                    ...
                    function (data) {
                        if (data.status) {
                            ...
                            if(typeof data.response.messages != 'undefined')
                                alert(data.response.messages[0]);
                            else
                ...
                error: function (data, x, y) {
                    $(comp).show();
                    $(that).css("opacity","1");
                    alert("Error occured while deleting the Plan! Please try again.");
                }
            });
        }
    }
