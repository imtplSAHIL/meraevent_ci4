<?php
$uri = service('uri');
$segments = $uri->getSegments(); // Get all segments as array

// Set defaults based on current page
$displayHeaderType = !empty($segments) ? $segments[0] : 'home';
$displayMethodType = isset($segments[1]) ? $segments[1] : '';
$createEditDisplayHeader = isset($segments[2]) ? $segments[2] : '';
...
foreach ($countryList as $value) { 
	...
	{
		?>
		<a class="btn headerDD" onclick="setCookieCustom('countryId', '<?php echo $value['id']; ?>', '<?php echo COOKIE_EXPIRATION_TIME;?>')" href="<?php echo $value['domain']; ?>">
            <span class="status">
