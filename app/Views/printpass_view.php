...
if(!empty($this->input->post())){
	...
	$userId = isset($eventsignupDetails['userid'])? $eventsignupDetails['userid']:$this->customsession->getData('userId');
	if(empty($enterEmail)){
           $enteredEmail=$eventsignupDetails[$userId];
        }     
        $barcodeNumber = $eventsignupDetails['barcodenumber'];
	$EventSignupId = $eventsignupDetails['id'];
	$venueName = $eventData['location']['venueName'].", ".$eventData['location']['cityName'];
...
<?php //foreach($attendees['otherAttendee'] as $key=>$val){ ?>
          ...
          <div class="col-md-4 col-sm-4 text-center">Reg No : 195245</div>
						...
						<div class="col-lg-8">
							...
							<span class="leftsection"><i class="icon icon-calender"></i>
								...
								if(isset($configCustomDatemsg[$eventId]) && !isset($configCustomTimemsg[$eventId])){
									...
									echo convertDate($convertedStartTime);if ($convertedStartTime != $convertedEndTime)  { echo " - ". convertDate($convertedEndTime).' '.$eventData['location']['timezone']; } echo  " | ".$configCustomTimemsg[$eventId];
							...
							if((isset($eventSettings['eventlocationhide']) && $eventSettings['eventlocationhide'] == 1) || $eventData['categoryid'] == 15){}
                                                            ...
                                                            <?php } else {?>
                                                                <span class="leftsection"><i class="icon icon-google_map_icon"></i><?php echo $eventData['fullAddress'];?></span>
                                                            <?php }
							}?>
						</div>
            ...
            <?php  //foreach($attendees['mainAttendee'] as $attndkey => $attndvalues){?>
                    ...
                    <?php //}?>
                    <p>Event ID : <?php if($eventData['multiEventParentId'] > 0){echo $eventData['multiEventParentId'];}else{echo $eventsignupDetails['eventid'];}?></p>
            ...
            <div class="row ticket_type">
						<div class="col-xs-6">
							<!--<p class="organized_by">
								Organized Byy : <span> <?php echo !empty($organizerDetails['company']) ? $organizerDetails['company'] : $organizerDetails['name'];?><br>Mobile : <?php echo !empty($eventData['eventDetails']['contactnumber']) ? $eventData['eventDetails']['contactnumber'] : $organizerDetails['mobile'];?></span>
						...
						<p class="TotalAmount_Ticket">Amount: <?php echo $ticketamount>0?$ticketamount." ".$eventsignupDetails['currencyCode']:0;?></p>
                                <?php if($totaldiscountamount>0 && $eventsignupDetails['mediscountid'] == 0){?>
              ...
              <?php }}?>
							<?php 
                                if($eventsignupDetails['eventextrachargeamount'] > 0 && $eventSettings['displaychargesonticket'] != 1){
                                    $eventsignuptotalamountinticket = $eventsignuptotalamountinticket - $eventsignupDetails['eventextrachargeamount'] - $eventsignupDetails['totalinternethandlingfee'];
                                }
                                ?>
                                <?php
                                    ...
                                    <p class="TotalAmount_Ticket">Convenience Fee :
                                        <?php
                                            if($internetHandlingFee > 0 ){
                                                echo round($internetHandlingFee)." ". $eventsignupDetails['currencyCode'] ;
                                            }else {
                                                echo '0';
                                            }
                                        ?><br>
                                                        ...
                                                        <?php } ?>
                                <?php if($eventsignupDetails['mediscountid'] != 0 && $eventsignupDetails['mediscountamount'] != 0){?>
                                    <p class="TotalAmount_Ticket">Discount :  <?php echo $eventsignupDetails['mediscountamount'];?> <?php echo $eventsignupDetails['currencyCode'];?></p>
                                <?php }?>
                                <p class="TotalAmount_Ticket">Total Amount :
                                    <?php
                                        if($eventsignuptotalamountinticket>0){
                                            if($eventsignupDetails['mediscountid'] != 0){
                                                echo round($eventsignuptotalamountinticket-$eventsignupDetails['mediscountamount'])." ".$eventsignupDetails['currencyCode'];
                                            } else {
				...
				<?php if($eventSettings['printonticket']==3){?>
                        <div class="col-lg-2 col-md-2 col-sm-2">
                            <?php $barcodeNumber =  $key.'&angle=270';?>
                            <img src="/barcode/barcode.php?text=<?php echo $barcodeNumber;?>" style="margin-top: 60px;" width="90" height="200">
                        </div>
...
var api_resendDelegateEmail = '<?php echo commonHelperGetPageUrl('api_resendDelegateEmail')?>';
</script>
