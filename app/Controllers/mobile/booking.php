...
class Booking extends REST_Controller {
     ...
     *          email - string
     *          mobile - integer
     *          POST variables those are coming from paymentGateway response
     * @return  gives the response status based on saving the payment response
     */
    public function updateMobilePayment_post() {
        
        $postArray = $this->post();
        $gateway = $postArray['paymentGateway'];
        if(isset($gateway) && $gateway == 'paytm'){
            $bookingResponse = $this->bookingHandler->paytmProcessingApi($postArray);
        }
        else if(isset($gateway) && $gateway == 'stripe'){
            $bookingResponse = $this->bookingHandler->stripeMobileProcessingApi($postArray);
        }
        else{
            if(isset($postArray['paymentGateway']) && $postArray['paymentGateway']!= '')
                $bookingResponse['response']['messages'] = 'paymentGateway Not Matching';
            else
                $bookingResponse['response']['messages'] = 'paymentGateway required';
            $bookingResponse['status'] = false;
            $bookingResponse['statusCode'] = STATUS_BAD_REQUEST;
