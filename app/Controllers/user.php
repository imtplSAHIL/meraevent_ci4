...
// Get user's login information
    ...
    public function login() {
		 ...
		 //var_dump($uid);var_dump($fbcode);exit;
...
//                                 $this->config->item('js_angular_path') . 'user/directives/homeDirectives.js', 
    ...
    //
        ...
        //print_r($user);exit;
        	try {
				$inputArray['socialtype'] = 'facebook';
				$inputArray['email'] = $user['EMAIL'];
				$inputArray['name'] = $user['FULLNAME'];
				$inputArray['socialid'] = $user['FBID'];
				$inputArray['profilepath'] = $user['fbprofilepicurl'];
				$userresponse = $this->userHandler->socialLogin($inputArray);
      	...
      	$redUrl= $this->input->get('redirect_url');
        if(strlen($redUrl)>0){
       $data['fbloginUrl'] =  $this->facebook->getFbloginUrl(array(
...
//        }
    ...
    public function signup() {
            ...
            if(count($userDataArray) <1 || !isset($userDataArray['City']) || $userDataArray['City'] == '' ){
                ...
                if($user_loc_det_status['status']){
                    ...
                    if ($user_loc_det['city_name'] != '' && $user_loc_det['city_name'] != '-') {
                        ...
                        $data['ipCity'] = $user_loc_det['city_name'];
                    }
                    if ($user_loc_det['region_name'] != '' && $user_loc_det['region_name'] != '-') {
                        $localityArr[] = $user_loc_det['region_name'];
                        $data['ipState'] = $user_loc_det['region_name'];
                    }
                    if ($user_loc_det['country_name'] != '' && $user_loc_det['country_name'] != '-') {
                        $localityArr[] = $user_loc_det['country_name'];
                        $data['ipCountry'] = $user_loc_det['country_name'];
                    }
                    $data['ipLocality'] = implode(',',array_unique($localityArr));
                }
            }
        }

        $data['cssArray'] = array(
            $this->config->item('css_public_path') . 'angular/angular',
        ...
        if ($user) {
        	try {
				...
				$inputArray['profilepath'] = $user['fbprofilepicurl'];
				$userresponse = $this->userHandler->socialLogin($inputArray);
				//echo "<pre>";print_r($socialLogin);exit;
    ...
    {
        $this->commonHandler->log_message("\n ".$this->commonHandler->get_client_ip()." User controlelr Bulk Upload processBulkUpload function called :: ".date('Y/m/d H:i:s'));
        ...
        $this->commonHandler->log_message("\n ".$this->commonHandler->get_client_ip()." User controller Process Bulk Upload Line 360 :: ".date('Y/m/d H:i:s'));
        $this->commonHandler->log_message(print_r($bulk_data, TRUE));
