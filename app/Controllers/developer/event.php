...
class Event extends REST_Controller {
    ...
    public function getEventDetails_post() {
        ...
        if ($resultArray['statusCode'] == STATUS_OK) {
            $timeZoneName = $resultArray['response']['details']['location']['timeZoneName'];
            $resultArray['response']['details']['location']['timeZone'] = $timeZoneName;

            $resultArray['response']['details']['startDate'] = convertTime($resultArray['response']['details']['startDate'], $resultArray['response']['details']['location']['timeZone'], true);
            $resultArray['response']['details']['endDate'] = convertTime($resultArray['response']['details']['endDate'], $resultArray['response']['details']['location']['timeZone'], true);
        }
        if (isset($inputArray['fullhtml']) && $inputArray['fullhtml'] == 1) { } else {
            $resultArray = $this->dataCustomFormat($resultArray);
        }
        $this->response($resultArray, $resultArray['statusCode']);
    }
   ...
   /*
    ...
    private function dataCustomFormat($inputData){
        if(isset($inputData['response']['details']['description']))
        {
            ...
            $inputData['response']['details']['description'] = $description;
        }
        return $inputData;
