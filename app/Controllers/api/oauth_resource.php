...
class Oauth_resource extends REST_Controller {
    ...
    public function verifyAccessTokenUser_get(){
	 $inputArray['userId'] = $this->accessTokenDetails['user_id'];
	 $profileHandler=new Profile_handler();
     $personalDetails = $profileHandler->getPersonalDetails($inputArray);  
     $resultArray = $personalDetails['response'];
     $this->response($resultArray, $personalDetails['statusCode']); 		
	}
