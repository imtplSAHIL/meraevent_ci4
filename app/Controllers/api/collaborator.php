...
class Collaborator extends REST_Controller {
    ...
    function collaboratorResendEmail_post() {
        $inputArray = $this->input->post();
        $output = $this->collaboratorHandler->sendCollaboratorEmail($inputArray);
        //var_dump($output); exit;
        $responseArray = array('response' => $output['response']);
        $this->response($responseArray, $output['statusCode']);
    }
    
    function deleteCollaborator_post() {
        $inputArray = $this->input->post();
        $output = $this->collaboratorHandler->deleteCollaboratorEmail($inputArray);
        //var_dump($output); exit;
        $responseArray = array('response' => $output['response']);
        $this->response($responseArray, $output['statusCode']);
    }
