...
class Common_requests extends REST_Controller {
	...
	/*clear memcache key*/
	public function clearMemcacheKey_post() {
        $input = $this->input->post();
		
		require_once(APPPATH . 'handlers/common_handler.php');
		$commonHandler = new Common_handler();
		
		$response = $commonHandler->clearMemcacheKey($input);
		
		if($response['status']){
			$this->data['status'] = TRUE;
            $this->data['response']['message'] = 'Given Memcache key reset successfully';
            $statusCode=200;
            $this->response($this->data,$statusCode);
		}
		
		$this->data['status'] = FALSE;
        $this->data['response']['message'] = SOMETHING_WENT_WRONG;
